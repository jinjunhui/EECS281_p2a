...
/*
struct cmp_alive{
    bool operator()(const Zomb_for_active &a, const Zomb_for_active &b){
...
int main(int argc,char**argv) {
    ...
    priority_queue<unsigned int,vector<unsigned int>,less<>> large2small; //largest on top
    priority_queue<unsigned int,vector<unsigned int>,greater<>> small2large;//smallest on top
        ...
        while(round<=round_input){
            ...
            while (arrows > 0) {
                ...
                while (true) {
                    ...
                    if (iterator_for_zomb->name==zombs.top()->name){
                        break;
                    }
                    else{
                        iterator_for_zomb++;
                    ...
                    if (iterator_for_zomb == zombs_create_order.end()) {
                        exit(1);
                    }
                }
                 */
                ...
                if (zombs.top()->health > arrows) {//undead
                    //iterator_for_zomb->health -= arrows;
                    //zombs.top()->health =iterator_for_zomb->health;
    ...
    while(!zombs.empty()&&alive){
        ...
        while (arrows > 0&&!zombs.empty()) {
            ...
            while (true) {
                if (iterator_for_zomb->name == zombs.top()->name
                    && iterator_for_zomb->health == zombs.top()->health
                    && iterator_for_zomb->speed == zombs.top()->speed
                    && iterator_for_zomb->distance == zombs.top()->distance)
                    break;
                else{
                    iterator_for_zomb++;
                }
             ...
             */
            if (zombs.top()->health > arrows) {//zomb is undead
    ...
    if(s!=0){
        ...
        //priority_queue<Zomb_for_active, vector<Zomb_for_active>, cmp_alive> zombs_active;
        //priority_queue<Zomb_for_active, vector<Zomb_for_active>, cmp_alive2>zombs_active2;
        ...
        while(!zombs.empty()){
            //zombs_active.push({zombs.top()->name,round-zombs.top()->round_generated});
            temp1->name=zombs.top()->name;
            temp1->active_round=round-zombs.top()->round_generated;
            active.push_back(temp1);
            active2.push_back(temp1);
            //active2.push_back({zombs.top()->name,round-zombs.top()->round_generated});
            zombs.pop();
        }
